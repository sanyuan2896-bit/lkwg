...
小程序会展示当前时间段的远行商人商品，并在命中指定商品时通过微信订阅消息提醒用户。订阅消息按微信规则计次：用户每授权一次，就增加一次可推送机会；后台成功发送一次提醒后，自动扣减一次。
...
定时检查最新商品快照。发现名称含「炫彩」、棱镜球、祝福项坠的商品后，给仍有订阅次数的用户发送订阅消息，并扣减 `remainingCount`。
